...
// expected errors are declared in test.
var TLogger *tLogger
...
func init() {
	...
	TLogger.v = config.WECROSS_GO_LOG_VERBOSITY_LEVEL
}

// getCallingPrefix returns the <file:line> at given depth from the stack.
...
// Update(). Note that if an expected error is not encountered, this wall cause
// the test to fail.
